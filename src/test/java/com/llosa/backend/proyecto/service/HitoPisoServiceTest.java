...
class HitoPisoServiceTest {
    ...
    // CP18: compra tardía — hitos anteriores al actual se completan en batch sin verificar precedencia
    @Test
    void marcarHitosAnterioresCompletados_marcaSoloLosAnteriores() {
        UUID activoId = UUID.randomUUID();
        Piso piso = Piso.builder().id(1L).nroPiso(1).build();
        Hito h1 = buildHito(1);
        Hito h2 = buildHito(2);
        Hito h3 = buildHito(3); // este es el "actual", no debe marcarse
        HitoPiso hp1 = buildHitoPiso(h1, piso, EstadoHito.PENDIENTE);
        HitoPiso hp2 = buildHitoPiso(h2, piso, EstadoHito.PENDIENTE);
        HitoPiso hp3 = buildHitoPiso(h3, piso, EstadoHito.PENDIENTE);

        when(hitoPisoRepository.findByActivoIdOrderByHitoOrdenAsc(activoId))
                .thenReturn(List.of(hp1, hp2, hp3));

        service.marcarHitosAnterioresCompletados(activoId, 3);

        assertThat(hp1.getEstado()).isEqualTo(EstadoHito.COMPLETADO);
        assertThat(hp2.getEstado()).isEqualTo(EstadoHito.COMPLETADO);
        assertThat(hp3.getEstado()).isEqualTo(EstadoHito.PENDIENTE); // no tocado
        verify(hitoPisoRepository).saveAll(argThat(list -> ((List<?>) list).size() == 2));
    }

    // CP18: hitos ya completados no se duplican en el batch
    @Test
    void marcarHitosAnterioresCompletados_noRescribeYaCompletados() {
        UUID activoId = UUID.randomUUID();
        Piso piso = Piso.builder().id(1L).nroPiso(1).build();
        Hito h1 = buildHito(1);
        Hito h2 = buildHito(2);
        HitoPiso hp1 = buildHitoPiso(h1, piso, EstadoHito.COMPLETADO); // ya completado
        HitoPiso hp2 = buildHitoPiso(h2, piso, EstadoHito.PENDIENTE);
        ...
        // solo hp2 debe ser guardado (hp1 ya estaba COMPLETADO)
        verify(hitoPisoRepository).saveAll(argThat(list -> ((List<?>) list).size() == 1));
    }

    // CP18: sin hitos pendientes anteriores no llama a saveAll
    @Test
    void marcarHitosAnterioresCompletados_sinPendientes_noGuarda() {
        UUID activoId = UUID.randomUUID();
        when(hitoPisoRepository.findByActivoIdOrderByHitoOrdenAsc(activoId))
                .thenReturn(List.of());

        service.marcarHitosAnterioresCompletados(activoId, 2);
    ...
    @Test
    void cambiarEstadoPorTorre_actualizaTodosLasUnidadesDelaTorre() {
        Long torreId = 1L;
        UUID hitoId = UUID.randomUUID();
        Piso piso = Piso.builder().id(1L).nroPiso(1).build();
        Hito hito = buildHito(1);
        hito = Hito.builder().id(hitoId).titulo("Casco").orden(1).tipo(TipoHito.OBRA)
                .estado(EstadoHito.PENDIENTE).build();
        HitoPiso hp1 = buildHitoPiso(hito, piso, EstadoHito.PENDIENTE);
        HitoPiso hp2 = buildHitoPiso(hito, piso, EstadoHito.PENDIENTE);

        when(hitoPisoRepository.findByTorreIdAndHitoId(torreId, hitoId))
                .thenReturn(List.of(hp1, hp2));
        when(hitoPisoRepository.saveAll(anyList())).thenAnswer(inv -> inv.getArgument(0));

        List<HitoPiso> result = service.cambiarEstadoPorTorre(torreId, hitoId, EstadoHito.COMPLETADO);

        assertThat(result).hasSize(2);
        assertThat(result).allMatch(hp -> hp.getEstado() == EstadoHito.COMPLETADO);
        assertThat(result).allMatch(hp -> hp.getFechaCompletado() != null);
    }

    // CP23: torre sin unidades para ese hito devuelve lista vacía
    @Test
    void cambiarEstadoPorTorre_sinHitos_devuelveListaVacia() {
        Long torreId = 99L;
        UUID hitoId = UUID.randomUUID();
        when(hitoPisoRepository.findByTorreIdAndHitoId(torreId, hitoId)).thenReturn(List.of());
